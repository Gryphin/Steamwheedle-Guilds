...
for i in $ash_mem
    do curl --create-dirs -o ~/guild-downloader/Ashamane/$i.json "https://api.warcraftrumble.gg/player/$i"
done
##Hook Point
curl --create-dirs -o Hook_Point.json https://api.warcraftrumble.gg/guild/200915262
hp_mem=$(jq '.[].members[].id' Hook_Point.json)
for i in $hp_mem
    do curl --create-dirs -o ~/guild-downloader/Hook_Point/$i.json "https://api.warcraftrumble.gg/player/$i"
done
##Bilgewater_Port
...
for i in $bbc_mem
    do curl --create-dirs -o ~/guild-downloader/Booty_Bay_Crew/$i.json "https://api.warcraftrumble.gg/player/$i"
done
##Kaboom Krew
curl --create-dirs -o Kaboom.json https://api.warcraftrumble.gg/guild/204596792
kab_mem=$(jq '.[].members[].id' Kaboom.json)
for i in $kab_mem
    do curl --create-dirs -o ~/guild-downloader/Kaboom/$i.json "https://api.warcraftrumble.gg/player/$i"
done
##Tiger's Peak
curl --create-dirs -o Tigers.json https://api.warcraftrumble.gg/guild/199002432
tp_mem=$(jq '.[].members[].id' Tigers.json)
for i in $tp_mem
    do curl --create-dirs -o ~/guild-downloader/Tigers/$i.json "https://api.warcraftrumble.gg/player/$i"
done
##Steamwheedle
curl --create-dirs -o Steamwheedle.json https://api.warcraftrumble.gg/guild/437262641
sw_mem=$(jq '.[].members[].id' Steamwheedle.json)
for i in $sw_mem
    do curl --create-dirs -o ~/guild-downloader/Steamwheedle/$i.json "https://api.warcraftrumble.gg/player/$i"
done
##Alchemist Union
curl --create-dirs -o Alchemist.json https://api.warcraftrumble.gg/guild/202672512
au_mem=$(jq '.[].members[].id' Alchemist.json)
for i in $au_mem
    do curl --create-dirs -o ~/guild-downloader/Alchemist/$i.json "https://api.warcraftrumble.gg/player/$i"
done
##For Hire
curl --create-dirs -o For_Hire.json https://api.warcraftrumble.gg/guild/466386371
fh_mem=$(jq '.[].members[].id' For_Hire.json)
for i in $fh_mem
    do curl --create-dirs -o ~/guild-downloader/For_Hire/$i.json "https://api.warcraftrumble.gg/player/$i"
done
##Viva le Beeyo
...
for i in $vlb_mem
    do curl --create-dirs -o ~/guild-downloader/Viva_le_Beeyo/$i.json "https://api.warcraftrumble.gg/player/$i"
done
##Tuck Hound Clan
...
for i in $tuk_mem
    do curl --create-dirs -o ~/guild-downloader/Tuck_Hound_Clan/$i.json "https://api.warcraftrumble.gg/player/$i"
done
##Everlook
curl --create-dirs -o Everlook.json https://api.warcraftrumble.gg/guild/438015331
el_mem=$(jq '.[].members[].id' Everlook.json)
for i in $el_mem
    do curl --create-dirs -o ~/guild-downloader/Everlook/$i.json "https://api.warcraftrumble.gg/player/$i"
done
##Spooky Syndicate
...
for i in $spsy_mem
    do curl --create-dirs -o ~/guild-downloader/Spooky_Syndicate/$i.json "https://api.warcraftrumble.gg/player/$i"
done
##Ring of Trials
...
for i in $rot_mem
    do curl --create-dirs -o ~/guild-downloader/Ring_of_Trials/$i.json "https://api.warcraftrumble.gg/player/$i"
done
##Fuselight
curl --create-dirs -o Fuselight.json https://api.warcraftrumble.gg/guild/439157421
rot_mem=$(jq '.[].members[].id' Fuselight.json)
for i in $rot_mem
    do curl --create-dirs -o ~/guild-downloader/Fuselight/$i.json "https://api.warcraftrumble.gg/player/$i"
done
##Ratchet
curl --create-dirs -o Ratchet.json https://api.warcraftrumble.gg/guild/437956461
r_mem=$(jq '.[].members[].id' Ratchet.json)
for i in $r_mem
    do curl --create-dirs -o ~/guild-downloader/Ratchet/$i.json "https://api.warcraftrumble.gg/player/$i"
done
##Circle of Blood
...
for i in $cob_mem
    do curl --create-dirs -o ~/guild-downloader/Circle_of_Blood/$i.json "https://api.warcraftrumble.gg/player/$i"
done
##Gallywix Palace
curl --create-dirs -o Gallywix.json https://api.warcraftrumble.gg/guild/439021421
gp_mem=$(jq '.[].members[].id' Gallywix.json)
for i in $gp_mem
    do curl --create-dirs -o ~/guild-downloader/Gallywix/$i.json "https://api.warcraftrumble.gg/player/$i"
done
##Cartel Overseers
...
for i in $co_mem
    do curl --create-dirs -o ~/guild-downloader/Cartel_Overseers/$i.json "https://api.warcraftrumble.gg/player/$i"
done
##Smokywood Pastures
...
for i in $swp_mem
    do curl --create-dirs -o ~/guild-downloader/Smokywood_Pastures/$i.json "https://api.warcraftrumble.gg/player/$i"
done
##The_Cartel
curl --create-dirs -o The_Cartel.json https://api.warcraftrumble.gg/guild/199066642
tc_mem=$(jq '.[].members[].id' The_Cartel.json)
for i in $tc_mem
    do curl --create-dirs -o ~/guild-downloader/The_Cartel/$i.json "https://api.warcraftrumble.gg/player/$i"
done